...
Anything that does not fit these shapes is not said. **As of 2026-09-24 neither shape can be filled: `scripts/converge.sh` says NOT_CONVERGED (open OQs and four OPEN DISCs), the interpreter lane has not run on the current code, and nothing has been measured (the timings in `perf/EXPERIMENTS.md` are exploratory). What may be said is the lane line the gates print — 1327 of 1327 cases on c-1t, c-8t and js (two of them pass only while the original's random draw matches the port: DISC-010, DISC-011), `All terms check.` with 73 laws — never that the port is complete, fast, certified or verified.**
