...
| S5.136 | The ready rendering is state-dependent: after a `close`, the freshly unblocked issue appears in the list with its own type badge (`scn_lifecycle` step 4 shows `1. [● P1] [feature] proj-170: Implement user auth`, which was absent from step 1) | `scn_lifecycle` steps 1 and 4 | `scn_lifecycle`, `scn_child_ids` |
...
| S5.190 | The two no-change rename paths print, without a `✓`: `Label '{old}' already has that name; no changes made.` when old and new are equal, and `Label '{old}' not found on any issues.` when nothing matched. Both exit 0 | `label.rs:147-159` | (cases to add: `label_rename_noop`, `label_rename_missing`) |
...
| S5.201 | `br version` prints one line: `br version {version} ({build})`, extended with ` ({branch}@{commit7})` when both are known, ` ({branch})` with branch only, ` ({commit7})` with commit only, where `{commit7}` is the first 7 characters of the commit hash. The pinned oracle prints `br version 0.6.0 (release) (v0.6.0@b1cfebe)` | `version.rs:117-133` | `version_plain` |
| S5.202 | The port cannot truthfully reproduce the original's build metadata; that is OQ-003 and resolves to a DISC, not to a clause here. The **shape** above is what the port must keep | `docs/OPEN_QUESTIONS.md` OQ-003 | `version_plain` |
...
| S5.209 | Capacity warnings, when any exist, follow the create/update report as `Warning: {text}` lines on stderr; no captured case configures capacity | `create.rs:231-235` | (no case: needs a configured capacity) |
...
| S5.250 | Output is state-dependent, and the scenario goldens are the contract for that: the same argv produces different bytes after a mutation (`ready` gains and loses rows, `epic status` flips its glyph and gains the eligibility line, `blocked` shrinks its header count, `stats` moves from `2.0 hours` to `23.2 days`) | `goldens/scenarios/lifecycle.scn` | `scn_lifecycle`, `scn_child_ids`, `scn_last_touched`, `scn_dup_title` |
