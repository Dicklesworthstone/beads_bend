...
|---|---|---|---|
| S1 argv grammar, usage and parse-error texts (S1.1–S1.67) | core `Cli.*`: argv → `Command` or `UsageError` | spec twin | golden (every `usage_*` case); closed golden laws for the short texts |
...
| `type Command`, `type GlobalFlags`, `type UsageError` (`core/cli.bend`) | S1.1–S1.67 | data | | one constructor per in-scope command form |
| `Cli.parse` and its helpers | S1.1–S1.67 | spec | consumes the argv list | returns `Result<UsageError, Command>`; texts verbatim from S1 |
...
| `Query.visible`, `Query.filter`, `Query.sort`, `Query.page`, `Query.search`, `Query.count`, `Query.stats` (`core/query.bend`) | S4.100–S4.156 | spec | `+store` | comparators are §6's |
