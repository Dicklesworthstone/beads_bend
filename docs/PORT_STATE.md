...
| field | value |
|---|---|
| phase | 3 reference port, early. Done: −1 fit screen; 0 truth pack (344 goldens, floor STABLE); 1 spec, first pass (899 clauses, spec-lint 0 findings, blind self-containment review 10/10); 2 architecture (arch-lint PASS, NUMERIC_PLAN, parity skeleton). Phase 1's second and third extraction passes and the capture of the extractors' proposed cases have NOT run (S12.4). Of the port: fourteen core modules (`port/core/`: `bytes`, `sha256`, `id`, `time`, `json`, `failure`, `model`, `decode`, `store`, `surface`, `cli`, `help`, `run`, `scaffold`) and the IO shell `port/main.bend` with two custom effects (`Sys.exit`, `Sys.cwd`). The argument parser is done for the captured surface: every refusal, tip, usage line and carried help text is the original's bytes. There is NO command yet: an accepted command line loads the store and then answers the port's own "not ported yet" failure (exit 1), so nothing passes by accident. **106 of 344 cases pass on c-1t and on js (the same 106)** |
| tier | T3 |
...
| c-1t at HEAD | `./scripts/quick-lanes.sh <out-dir>` (its first line; it builds `port/main.bend` and runs `scripts/conform.sh … --lane c-1t` through the sandbox) | `{"lane": "c-1t", "passed": 106, "failed": 238, "inconclusive": 0, "stderr_compared": true, "verdict": "FAIL"}` | 2026-09-20 |
| js at HEAD | `./scripts/quick-lanes.sh <out-dir>` (its second line: `scripts/conform.sh … --lane js` on the bun build) | `{"lane": "js", "passed": 106, "failed": 238, "inconclusive": 0, "stderr_compared": true, "verdict": "FAIL"}` | 2026-09-20 |
| proofs | `(cd port && $BEND_CLI PROOF.bend)` | `All terms check.` (0 @unsafe; 37 laws: 3 SHA-256 vectors, 7 captured ids, the id-length table, 15 timestamp laws, the store line of `create_min`, the `malformed` fixture's refusal, 8 argument-parser laws `cli_*`, the scaffold's placeholder; 33 s; bend 2.0.20) | 2026-09-20 |
| law coverage | `./scripts/law-coverage.sh` | `{"laws": 37, "proofs": 37, "unproved": "", "ghost_proofs": "", "ghost_cited": "", "uncited": "fast_is_spec", "duplicate_laws": [], "duplicate_proofs": [], "unsafe": 0, "unsafe_annotations": 0, "verdict": "OK"}` | 2026-09-20 |
...
| parity | `./scripts/parity-board.sh docs/FEATURE_PARITY.md` | `{"rows": 42, "present": 0, "partial": 3, "missing": 26, "excluded": 13, "na": 0, "no_evidence": 0, "verdict": "PARTIAL"}` | 2026-09-20 |
...
`(cd port && $BEND_CLI PROOF.bend)` must still print `All terms check.`; then backlog E4 and E5 in `port/core/`: finish `store.bend` (the load repairs of S2.31–S2.36, the duplicate-id refusal, validation), then `query.bend` and `render.bend` for `list` and `count` in Plain, JSON and Quiet (read S4.100–S4.136, S5.20–S5.58, S5.120–S5.129 in `docs/EXISTING_BEADS_RUST_STRUCTURE.md`), dispatching in `Run.ran` on `Cli.Command{path, opts, words}`; first targets `empty_list`, `empty_list_json`, `empty_count`, `list_plain`, `list_json`, `usage_value_eq_form`, `usage_short_combined`; check with `./scripts/quick-lanes.sh <out-dir>`.
