...
| field | value |
|---|---|
| phase | 3 reference port, just begun. Done: −1 fit screen; 0 truth pack (235 goldens, floor STABLE); 1 spec, first pass (874 clauses, spec-lint 0 findings, blind self-containment review 10/10); 2 architecture (arch-lint PASS, NUMERIC_PLAN, parity skeleton). Phase 1's second and third extraction passes and the capture of the proposed cases have NOT run (S12.4). Of the port, three core modules exist (`port/core/bytes.bend`, `sha256.bend`, `id.bend`: UTF-8, SHA-256, the id hash) with 10 proved closed laws. There is no CLI: `port/main.bend` is still the scaffold template, so every conformance case fails |
| tier | T3 |
...
| proofs | `(cd port && $BEND_CLI PROOF.bend)` | `All terms check.` (0 @unsafe; 11 laws: 3 SHA-256 vectors, 7 captured ids, the scaffold's placeholder; bend 2.0.20) | 2026-09-20 |
| law coverage | `./scripts/law-coverage.sh` | `{"laws": 11, "proofs": 11, "unproved": "", "ghost_proofs": "", "ghost_cited": "", "uncited": "fast_is_spec", "duplicate_laws": [], "duplicate_proofs": [], "unsafe": 0, "unsafe_annotations": 0, "verdict": "OK"}` | 2026-09-20 |
...
`(cd port && $BEND_CLI PROOF.bend)` must still print `All terms check.`; then write `port/core/time.bend` for S4.52–S4.64 (split instant `{neg, secs, nanos}`, RFC 3339 parse and print with the 0/3/6/9 fraction rule) with the `precision` fixture's five spellings as closed laws, then `port/core/json.bend` (S2.24–S2.42, S5.1–S5.25). Module order and the shell's custom effects (`Clock.wall`, `Sys.cwd`, `Sys.remove`, `Sys.rename`, `Sys.exit`) are in `docs/PROPOSED_ARCHITECTURE.md` §2.
