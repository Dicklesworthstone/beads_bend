<!-- DRAFT of README.md (2026-09-20). Links are relative to the repository root. Every number was
     read from an artifact in this repository on that date: re-read them before publishing. -->
...
| Spec (Phase 1) | **in progress**: section extracts are landing under `docs/spec-parts/` (seven are planned); the merged spec is still the S1–S12 skeleton | `docs/EXISTING_BEADS_RUST_STRUCTURE.md` |
...
| Feasibility probes | a wall-clock custom effect (result recorded as OQ-009) and a SHA-256 over `U32` words with its expected vectors (no result recorded in any register, so nothing is claimed about it). Probes are not the port | `port/probes/` |
