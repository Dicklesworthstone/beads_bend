...
| timestamps: parse, order, print | S4.52–S4.64, S7.14 | `Time.*` | `edge_precision_ready`, `edge_precision_sort_created`, `edge_precision_sort_title` | none | missing | not started: Phase 3; planned laws (PROPOSED_ARCHITECTURE §8, not yet stated): time_roundtrip, time_precision_spellings |
| id generation and child ids | S4.1–S4.27, S7.1–S7.9 | `Sha.digest`, `Id.*` | `scn_dup_title`, `scn_child_ids` | `sha_empty`, `sha_abc`, `sha_two_blocks` | missing | not started: Phase 3; planned laws (PROPOSED_ARCHITECTURE §8, not yet stated): id_fyw, id_length_table |
