...
[![Status](https://img.shields.io/badge/status-Phase%203%3A%20every%20corpus%20command%20runs%2C%20381%20of%20381%20cases-orange.svg)](#status)
...
| Conformance cases | 381 cases in `goldens/cases.tsv` (8 added 2026-09-20 for the label-rename, typed dep-remove, single-parent, `-l` label and comment-text forms); `scripts/cases-lint.sh` verdict `OK` (0 errors, 0 notes) | `goldens/` |
| Goldens | 381 captured `.out` / `.err` / `.exit` triples from the pinned original; digests in `goldens/MANIFEST.txt`; 70 of them carry a dump of the resulting `.beads/issues.jsonl`. Three (`version_plain`, `version_json`, `usage_version_flag`) are compared through the DISC-006 shape canonicalizer, which reduces both sides to `<name> version <version> <details>` | `goldens/` |
| Reproducibility floor | `{"repeat":3,"stable":381,"unstable":[],"inconclusive":[],"oracle_identity_checked":true,"verdict":"STABLE"}`, after the one hash-random output of the original was canonicalized (DISC-005) | `docs/PORT_STATE.md` |
| Fixtures and scenarios | 7 fixtures (`basic`, `basic_touched`, `conflict`, `empty`, `large`, `malformed`, `precision`) and 5 multi-step scenarios; the realistic fixtures are the original's own output | `goldens/fixtures/`, `goldens/scenarios/` |
...
At this commit the three fast lanes report `PASS` with 381 of 381 cases each (c-1t, c-8t, js). The interpreter lane is not run over the whole corpus: it type-checks the book on every case, 2 min 28 s per run at this size, so the 381 cases would take about 14.5 hours; a 12-case sample across the command families passes 12 of 12 and stands in until a long checkpoint run is scheduled, and the gpu lane is `MISSING` (no device).
...
| Evidence of behavior | Its own test suites | 381 captured cases in this repository | Those same 381 cases on three lanes (c-1t, c-8t, js), plus 50 laws |
...
|------|---------|-------------|
| Case table syntax | `scripts/cases-lint.sh goldens/cases.tsv` | `OK`: 381 cases, 0 errors, 0 notes |
...
| Floor | `scripts/floor.sh goldens/cases.tsv goldens --repeat 3 --timeout 60 -- scripts/ws-run.sh --oracle br --no-db ::` | `STABLE`, 381 of 381 |
| Laws | `(cd port && $BEND_CLI PROOF.bend)` | `All terms check.` (0 `@unsafe`; 50 closed laws) |
| Lanes | `LANE_WRAP=$PWD/scripts/ws-run.sh scripts/lanes.sh goldens/cases.tsv goldens "$PWD/port/main.bend" --threads 8 --timeout 60 --interpreter-timeout 600` | `PASS` on c-1t, c-8t and js (381 of 381 each, run lane by lane with `scripts/conform.sh`); the interpreter lane needs about 14.5 hours for the corpus and is sampled instead (`PASS` 12/12); gpu `MISSING` (no device) |
...
└──────────────────────────────────────────────────────────────────────┘
                 ...
                 ▼
┌──────────────────────────────────────────────────────────────────────┐
│  goldens/   381 cases: <case>.out .err .exit, MANIFEST.txt           │
└──────────────────────────────────────────────────────────────────────┘
...
| DISC-006 | Platform | `br version` reports its Rust toolchain, target triple and git branch (`br version 0.6.0 (release) (v0.6.0@b1cfebe)`) | `bn version 0.1.0 (bend 2.0.20) (port of br 0.6.0@b1cfebe)`, and a JSON report with the same facts; the three cases are compared through a shape canonicalizer applied to both sides | None: there is no truthful way to print another program's build metadata | 3 of 381 cases are compared by shape rather than byte for byte |
| DISC-007 | BugFix | `br --no-db --no-db list` is refused (a once-only option twice) | An explicit top-level `--no-db` is accepted, so a script that calls `br --no-db list` keeps working when the port stands in | None | 0 of 381 |
| DISC-008 | Platform | `br --no-db count` answers on a store of any size (its own is 4.5 MB) | Was: the JS lane faulted above about 30 KB of store. **RESOLVED** the same day — the cause was the port's own shell joining its read chunks with `List.concat`, which the emitted JavaScript walks one stack frame per element; the shell now answers a single chunk as it is and reads 64 MiB at a time, and the JS lane carries 5,000 records | not applicable (repaired, not diverged) | 0 of 381 cases; the repair is locked in by fixture `large` and the three `edge_large_*` cases |
...
| **Conformance is bounded by the corpus** | 381 cases is what "golden-tested" means here; behavior outside them is unclaimed, and the forms the port refuses with its own failure are listed in `docs/OPEN_QUESTIONS.md` |
