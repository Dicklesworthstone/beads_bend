...
[![Status](https://img.shields.io/badge/status-Phase%203%3A%201007%20of%201009%20cases%20on%20c--1t%2C%20c--8t%2C%20js-orange.svg)](#status)
...
As of 2026-09-24 (commit `fd0a945`) this project is at the end of Phase 3 of an eight-phase port method (fit screen, truth pack, spec, architecture, reference port, parity gate, performance, certify). The fit screen, the truth pack, a first pass of the spec and the architecture are done; the reference port answers the commands the 1009-case corpus exercises, reads and writes the store, and reproduces the pinned original byte for byte on 1007 of 1009 cases on three of the four lanes (the interpreter lane has not run on the current code). Eleven find-fix rounds have run, one of them by a non-author; the eleventh (the real-store sweep) found three behaviors, so the two clean rounds tier T3 asks for at the end are yet to come; `scripts/converge.sh` still reports `NOT_CONVERGED`, because 21 spec questions (OQs) are still open and four DISCs are `OPEN` (DISC-009 to DISC-012), which only the repository owner can accept. No parity claim, no measurement, no certification. I would rather publish an honest account of a half-finished port than a README that describes software that does not exist.
...
| Conformance cases | 1009 cases in `goldens/cases.tsv` (16 added 2026-09-24 from what the real-store sweep found, OQ-131 to OQ-133; the OQ batches and find-fix rounds of 2026-09-22 and 2026-09-23 before them; 40 added 2026-09-22: `dep list --direction up/both`, `dep tree --format mermaid`, `blocked --detailed`, and `ready --parent/--recursive/--epic` on a new oracle-built fixture `parents`; 30 added 2026-09-21 for the dependency graph: real tree depth, a diamond, an ancestor back edge, `repeat` and `truncated`, `--max-depth`, the `up` and `both` directions, and the BLOCKED relation with two blockers and a blocking cycle); `scripts/cases-lint.sh`: `{"cases": 1009, "errors": 0, "notes": 9, "classes_missing": "", "verdict": "OK"}` | `goldens/` |
| Goldens | 1009 captured `.out` / `.err` / `.exit` triples from the pinned original; digests in `goldens/MANIFEST.txt`; 230 of them carry a dump of the resulting `.beads/issues.jsonl`. Three (`version_plain`, `version_json`, `usage_version_flag`) are compared through the DISC-006 shape canonicalizer, which reduces both sides to `<name> version <version> <details>` | `goldens/` |
...
| Spec (Phase 1) | **first pass done**: 909 numbered clauses with `file:line` provenance and a golden case each (25 of them, S1.68–S1.92, written from measured output while the argument parser was built), merged from eight part files by `scripts/merge-spec-parts.py`; `scripts/spec-lint.py`: `spec-lint: 909 clauses, 1009 cases, 1009 cases cited, 0 finding(s)`; a blind self-containment review predicted 10 of 10 sampled cases byte for byte from the spec alone (`docs/reviews/self-containment-1/`) and still found six clause contradictions, now open questions. The method's second and third extraction passes and the capture of the extractors' proposed cases have not run | `docs/EXISTING_BEADS_RUST_STRUCTURE.md`, `docs/spec-parts/`, `docs/OPEN_QUESTIONS.md` |
...
| Parity board (Phase 4) | `scripts/parity-board.sh`: `{"rows": 42, "present": 9, "partial": 20, "missing": 0, "excluded": 13, "na": 0, "no_evidence": 0, "verdict": "PARTIAL"}`. Eleven find-fix rounds have run (6 clean; round 10 by a non-author; round 11, the real-store sweep, found three behaviors, so the clean tail restarts at 0); `scripts/converge.sh` reports `NOT_CONVERGED`: 21 open OQs and four `OPEN` DISCs (DISC-009 to DISC-012) remain | `docs/FEATURE_PARITY.md` |
...
At commit `fd0a945` (2026-09-24) the three fast lanes, each run on its own with `scripts/conform.sh` on one build of the working tree, pass 1007 of 1009 cases each (c-1t, c-8t, js); the two failures are `prefix_config_both` (DISC-010) and `delete_hard_cascade_json` (DISC-011), whose goldens hold one of the values the original draws at random. `scripts/lanes.sh` as a whole has not run on this code (a BLOCKER in `docs/PORT_STATE.md`): its interpreter lane type-checks the whole program on every case and needs the host alone for hours. The gpu lane is `MISSING` (no device).
...
| Evidence of behavior | Its own test suites | 1009 captured cases in this repository | 1007 of those 1009 cases on three lanes (c-1t, c-8t, js; the other two are the original's random draws), plus 68 closed laws |
...
Results are the gate lines of 2026-09-24 (commit `fd0a945`); `docs/PORT_STATE.md` holds the pasted copies.

| Gate | Command | Last result |
|------|---------|-------------|
| Case table syntax | `scripts/cases-lint.sh goldens/cases.tsv` | `{"cases": 1009, "errors": 0, "notes": 9, "classes_missing": "", "verdict": "OK"}` |
...
| Laws | `(cd port && $BEND_CLI PROOF.bend)` | `All terms check.` (0 `@unsafe`; 68 closed laws; `bend 2.0.20`) |
| Lanes | `LANE_WRAP=$PWD/scripts/ws-run.sh scripts/lanes.sh goldens/cases.tsv goldens "$PWD/port/main.bend" --threads 8 --timeout 60 --interpreter-timeout 600` | BLOCKER: `lanes.sh` as a whole has not run on this code. c-1t, c-8t and js, run lane by lane with `scripts/conform.sh` on one build each at `fd0a945`, pass 1007 of 1009 each; the two failures are `prefix_config_both` (DISC-010) and `delete_hard_cascade_json` (DISC-011). The interpreter lane has not run on the current code (it needs the host alone for hours); gpu `MISSING` (no device) |
...
| Convergence | `scripts/converge.sh docs/PORT_STATE.md` | `NOT_CONVERGED`: rounds 11, clean 6, clean tail 0 (round 11 found three behaviors), a non-author round; 21 open OQ and 4 open DISC (DISC-009, DISC-010, DISC-011, DISC-012) remain, and only the repository owner can accept a DISC |
...
| `ready` | Open, unblocked, not deferred work; `--sort priority\|oldest`, `--include-deferred`, filters; `--parent <id>` keeps its direct children, `-r`/`--recursive` all its descendants, `--epic <id>` is both | `ready --epic proj-9vo --json` | 28 |
| `blocked` | Issues with open blocking dependencies; `--detailed` lists each blocker's title, priority and status | `blocked --detailed` | 9 |
...
| Command | Contract (`br --no-db`) | Example (captured) | Cases |
|---------|-------------------------|--------------------|-------|
| `dep add` / `dep remove` | Typed edges; refuses cycles (exit 5), self-dependency, missing ids, a second parent; duplicate and `external:` edges | `dep add proj-ptp proj-mta -t related` | 12 / 3 |
| `dep list` / `dep tree` / `dep cycles` | Read the graph. `dep tree` walks depth-first in pre-order, orders siblings by priority, status rank, title and id, marks a re-reached node `repeat` and a cut one `truncated`, and takes `--max-depth` (default 10) and `--direction`; `--format mermaid` prints the same walk as a mermaid graph. `dep list --direction up\|both` lists dependents too | `dep tree proj-6ja --format mermaid` | 15 / 31 / 4 |
...
|-----------|-------|------------------------|
| 0 | 647 | success, including no-op outcomes such as `dep_add_duplicate` (`Dependency already exists: …`) and the `delete` preview; every multi-step scenario |
| 2 | 104 | argument-parser usage errors; `NOT_INITIALIZED` (no `.beads/`) |
| 3 | 35 | `ISSUE_NOT_FOUND`, `AMBIGUOUS_ID`, `NOTHING_TO_DO` |
| 4 | 87 | `VALIDATION_FAILED`, `INVALID_PRIORITY`, no ids and no last-touched issue |
| 5 | 10 | `CYCLE_DETECTED`, self-dependency |
| 6 | 9 | `SYNC_CONFLICT`: the import's semantic verification refuses a record (`edge_empty_optionals_list`) |
| 7 | 109 | `CONFIG_ERROR`: merge conflict markers or malformed JSON in `issues.jsonl`, and the other configuration refusals |
| 8 | 8 | `IO_ERROR`: a store that is not UTF-8, a file an argv names that is missing (`error_store_not_utf8`, `error_comments_add_file_missing`) |
...
└──────────────────────────────────────────────────────────────────────┘
                 ...
                 ▼
┌──────────────────────────────────────────────────────────────────────┐
│  goldens/   1009 cases: <case>.out .err .exit, MANIFEST.txt          │
└──────────────────────────────────────────────────────────────────────┘
...
| **Conformance is bounded by the corpus** | 1009 cases is what "golden-tested" means here; behavior outside them is unclaimed, and the forms the port refuses with its own failure are listed in `docs/OPEN_QUESTIONS.md` |
...
No. You can build it (`bend port/main.bend -o bn`) and it reproduces the original on 1007 of the 1009 captured cases (the other two are outputs the original draws at random), but that is a corpus, not a certification: the parity gate has not converged (21 open OQs, four `OPEN` DISCs), the interpreter lane has not run on the current code, the performance phase and the report have not run, the exclusions below are real (no `beads.db`, no locks, no atomic write-back), and several uncaptured forms answer the port's own "not ported" failure instead of guessing. Use [`br`](https://github.com/Dicklesworthstone/beads_rust).
